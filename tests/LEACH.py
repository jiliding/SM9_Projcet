...
class WSN(object):
    ...
    def trans_energy(data, dis):
        ...
        return energy
    ##判断节点死亡个数，第一个节点死亡时间和轮数，所有节点死亡时间和轮数
    def node_state(r):
        nodes  = WSN.nodes
        n_dead = 0
        nodes_dead = []
        ...
        WSN.n_dead = n_dead # 死亡节点个数
        WSN.nodes_dead = nodes_dead # 死亡节点列表信息
...
class Node(object):
    ...
    trans_delay_init = 40  # 初始传输时延，数据包长度4096除以100kb/s
    Trans_Rate_init = 100  # 传输速率为100kb/s
    Attack_probability = 0.8  # 隐藏恶意节点攻击概率为0.8，持续攻击恶意节点攻击概率为1
    credibility_threshold = 0.5  # 信誉值阈值，高于此值为正常节点，否则为异常节点
    # self.trans_delay = None  # 传输时延
    # self.Packet_integrity = None  # 数据包完整性
    # self.Trans_Rate = None  # 数据包传输速率
    def __init__(self):
        ...
        # self.Packet_integrity = None  # 数据包完整性
        # self.Trans_Rate = None  # 数据包传输速率
        self.Normal_communicate = 0 # 正常通信次数a
        self.Abnormal_communicate = 0 # 异常通信次数b
        self.credibility = 0 # 信誉值，信誉值计算公式为（a+1）/(a+b+2)
    ...
    def plot_wsn():
        ...
        # plt.pause(1)
        # plt.close('all')
        # plt.savefig("./image_jpg/0WSN_initial.jpg")
        plt.savefig("./image_png/0WSN_initial.png")
        plt.close('all')
...
class Leach(object):
    ...
    r       = 0 # 当前轮数
    rmax    = 800#9999 # default maximum round
    r_empty = 0 # 空轮
    leach_alive = []
    empty_round = 0
    def show_cluster():
        ...
        plt.xlim((-10, 210))
        plt.ylim((-10, 210))
        plt.title("第{}轮分簇结果".format(Leach.r))
        icon = ['o', '*', '.', 'x', '+', 's']
        color = ['r', 'b', 'g', 'c', 'y', 'm']
        # 对每个簇分类列表进行show
        i = 0
        ...
        # plt.show()
        #plt.draw()
        # plt.savefig("./image_jpg/LEACH-{:0>4d}.jpg".format(Leach.r))
        plt.savefig("./image_png/LEACH-{:0>4d}.png".format(Leach.r))
        # 以下为测试
        # plt.show()
        # plt.pause(1)
        # plt.close('all')
    ...
    def run_leach():
        for r in range(Leach.rmax):
            ...
            if WSN.flag_net_stop:
                ...
                break
            Leach.show_cluster()
            Leach.leach_alive.append(int(WSN.n-WSN.n_dead))
        fig = plt.figure()
        ...
        # axs_x = []
        # count = 1
        # for item in Leach.leach_alive:
        #     axs_x.append(count)
        #     count += count
        plt.plot(Leach.leach_alive)
        plt.savefig("./image_png/LEACH_alive.png")
        plt.close()
